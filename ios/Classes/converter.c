...
struct imageResult convertImage(uint8_t *plane0, uint8_t *plane1, uint8_t *plane2, int bytesPerRow, int bytesPerPixel, int width, int height){
    int hexFF = 255;
    ...
    int data = 2;
    struct imageResult resultStruct ;
    resultStruct.image = image;
    resultStruct.result = 2;
    return resultStruct;
}
